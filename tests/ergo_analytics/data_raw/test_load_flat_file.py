...
def test_base_data():
    ...
    date_values_after_load = list(map(lambda x: str(x).split("T")[0], data['Date-Time'].values))
    ...
    assert list(data['DeltaPitch'].values) == list(map(float, fake_data['dpitch'].values))
