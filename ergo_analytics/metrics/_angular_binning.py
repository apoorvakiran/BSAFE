...
def angular_binning(
    ...
    if "yaw" not in exclude_angles:
        raw_score_yaw = compute_binned_score(
            bins=bins_degrees, values=delta_yaw, weighing_method=weighing_method
        )
        total_count += 1
    else:
        raw_score_yaw = 0
    ...
    if "pitch" not in exclude_angles:
        raw_score_pitch = compute_binned_score(
            bins=bins_degrees, values=delta_pitch, weighing_method=weighing_method
        )
        total_count += 1
    else:
        raw_score_pitch = 0
    ...
    if "roll" not in exclude_angles:
        raw_score_roll = compute_binned_score(
            bins=bins_degrees, values=delta_roll, weighing_method=weighing_method
        )
        total_count += 1
    else:
        raw_score_roll = 0
    ...
    strain_scores = dict(
        yaw_raw=raw_score_yaw, pitch_raw=raw_score_pitch, roll_raw=raw_score_roll
    )
    ...
    if total_score_method == "average":
        ...
        strain_scores["total"] = (
            strain_scores["yaw"] + strain_scores["pitch"] + strain_scores["roll"]
        ) / total_count
