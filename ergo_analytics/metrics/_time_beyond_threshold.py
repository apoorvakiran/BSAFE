...
def time_beyond_threshold(
    ...
    if delta_pitch is None or delta_yaw is None or delta_roll is None:
        msg = (
            "one or more of the incoming delta-angles is None!"
    ...
    score_yaw_raw = compute_binned_score(
        bins=bins, values=delta_yaw, weighing_method="linear"
    )
    score_pitch_raw = compute_binned_score(
        bins=bins, values=delta_pitch, weighing_method="linear"
    )
    score_roll_raw = compute_binned_score(
        bins=bins, values=delta_roll, weighing_method="linear"
    )

    # summarize posture scores
    posture_scores = dict(
        yaw_raw=score_yaw_raw,
