...
def time_beyond_threshold(
    delta_pitch=None, delta_yaw=None, delta_roll=None, safe_threshold=None, final_scale=(0, 7), exclude_angles=None
):
    ...
    num_unsafe = len(
        np.where(
            (absolute(delta_pitch) > safe_threshold)
            | (absolute(delta_yaw) > safe_threshold)
            | (absolute(delta_roll) > safe_threshold)
        )[0]
    )
    num_total = len(delta_yaw)

    bins = [safe_threshold, 180]
    score_yaw_raw = compute_binned_score(bins=bins, values=delta_yaw, weighing_method="linear")
    score_pitch_raw = compute_binned_score(bins=bins, values=delta_pitch, weighing_method="linear")
    score_roll_raw = compute_binned_score(bins=bins, values=delta_roll, weighing_method="linear")

    # summarize posture scores
    posture_scores = dict(
        yaw_raw=score_yaw_raw, pitch_raw=score_pitch_raw, roll_raw=score_roll_raw, unsafe_raw=num_unsafe / num_total
    )
    # the raw scores are in the range (0, 1)
    # now convert to (0, 7):

    posture_scores["yaw"] = normalize_to_scale(
        old_lo=0, old_hi=1, new_lo=final_scale[0], new_hi=final_scale[1], values=posture_scores["yaw_raw"]
    )

    posture_scores["pitch"] = normalize_to_scale(
        old_lo=0, old_hi=1, new_lo=final_scale[0], new_hi=final_scale[1], values=posture_scores["pitch_raw"]
    )

    posture_scores["roll"] = normalize_to_scale(
        old_lo=0, old_hi=1, new_lo=final_scale[0], new_hi=final_scale[1], values=posture_scores["roll_raw"]
    )

    posture_scores["unsafe"] = normalize_to_scale(
        old_lo=0, old_hi=1, new_lo=final_scale[0], new_hi=final_scale[1], values=posture_scores["unsafe_raw"]
    )
