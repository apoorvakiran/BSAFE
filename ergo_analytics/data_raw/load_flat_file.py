...
class LoadDataFromLocalDisk(BaseData):
    ...
    def _read_datafile(self, path=None, data_format_code=None):
        """
        ...
        if data_format_code is None:
            data_format_code = self._find_data_format_code(path=path)
        ...
        data = self._find_numeric_and_correct_columns(
            data, data_format_code=data_format_code
        )
        ...
        data = self._cast_to_correct_types(
            all_data=data, data_format_code=data_format_code
        )

        return data

    def get_data(self, path=None, destination=None, data_format_code=None):
        """
        ...
        while True:
            try:
                ...
                if not is_numeric(val) or (
                    np.isnan(float(val)) or data.iloc[ix].isna().any()
                ):
