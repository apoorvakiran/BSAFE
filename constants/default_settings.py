...
__all__ = ["DATA_FORMAT_CODES", "FRACTION_OF_DATA_USEFUL", "SAMPLING_RATE", "DATE"]
...
def convert_date(date_from_device=None):
    ...
    """
    if isinstance(date_from_device, pd.Series):
        return pd.to_datetime(date_from_device)
...
ax[0]
   ...
   az[1]
      ...
      delta_pitch,
      delta_roll""".split()
COLUMN_NAMES_FORMAT_3 = list(
    map(lambda x: x.strip().rstrip(",").lstrip(","), COLUMN_NAMES_FORMAT_3)
)
COLUMN_NUMERICS_FORMAT_3 = COLUMN_NAMES_FORMAT_3.remove("Date-Time")
COLUMN_TYPES_FORMAT_3 = [
    DATE,
    float,
    float,
    float,
...
COLUMN_TYPES_FORMAT_4 = [DATE, float, float, float]

# JTK: Added October 21, 2019:
COLUMN_NAMES_FORMAT_5 = [
    "Date-Time",
    "Yaw[0](deg)",
    "Pitch[0](deg)",
    "Roll[0](deg)",
